...
describe("the limiter", () => {
  const limiter = () => new RateLimiter(new MemorySharedStore());

  it("allows up to the limit and refuses the next one", async () => {
    const rl = limiter();
    for (let i = 0; i < 3; i++) {
      expect((await rl.check("k", 3, 1000)).ok, `hit ${i + 1} should pass`).toBe(true);
    }
    const refused = await rl.check("k", 3, 1000);
  ...
  it("keeps separate keys separate", async () => {
    const rl = limiter();
    await rl.check("a", 1, 1000);
    // One person hitting their limit must never lock out anybody else.
    expect((await rl.check("b", 1, 1000)).ok).toBe(true);
  });

  it("lets the caller back in once the window passes", async () => {
    const rl = limiter();
    expect((await rl.check("k", 1, 20)).ok).toBe(true);
    expect((await rl.check("k", 1, 20)).ok).toBe(false);
    await new Promise((r) => setTimeout(r, 40));
    expect((await rl.check("k", 1, 20)).ok).toBe(true);
  });

  it("shares one window across instances", async () => {
    // The reason this moved out of process memory. Two instances used to keep
    // separate counts, so a cap of 3 became 3 x the instance count — on the
    // endpoint that sends a real WhatsApp message to a number in the request
    // body. Two limiters over one store must now agree.
    const store = new MemorySharedStore();
    const instanceA = new RateLimiter(store);
    const instanceB = new RateLimiter(store);

    expect((await instanceA.check("shared", 2, 60_000)).ok).toBe(true);
    expect((await instanceB.check("shared", 2, 60_000)).ok).toBe(true);
    // The third hit is refused no matter which instance receives it.
    expect((await instanceA.check("shared", 2, 60_000)).ok).toBe(false);
    expect((await instanceB.check("shared", 2, 60_000)).ok).toBe(false);
  });
});
